...
const LASTMOD = path.join(ROOT, 'src/data/lastmod.json');
...
/* --- lastmod, derived from content rather than from the clock ---------------
   THE BUG THIS REPLACES. `lastmod` used to be the build date, so every rebuild
   told crawlers that all seven pages had changed — including a rebuild that
   changed nothing, which is most of them. Search engines discount a sitemap
   whose lastmod is always "today", so the field was not merely wrong, it was
   spending the credibility it exists to earn.

   Found the ordinary way: a verification rebuild after midnight produced a
   seven-line diff on a day when no content had changed at all.

   HOW IT WORKS NOW. Each page's built bytes are hashed and the hash is stored
   with the date it was first seen. A page whose hash is unchanged keeps its
   old date however many times it is rebuilt; a page whose bytes actually
   change gets today. The store is committed, which is what makes the date
   survive a fresh clone — and it makes the sitemap deterministic, so CI can
   check it like every other built file rather than excluding it.
...
const lastmodMap = (stamp) => {
  let store = {};
  try {
    store = JSON.parse(fs.readFileSync(LASTMOD, 'utf8'));
  } catch {
    /* FIRST RUN — seed from the sitemap already on disk rather than from the
       clock. The old sitemap's dates are wrong in the specific way this change
       fixes (they are build dates), but for the LAST build they are also the
       right answer, because that build is when the pages last changed. Seeding
       from today instead would announce that all seven pages changed on the
       day the mechanism was installed, which is the exact false claim being
       removed. Pages with no previous entry fall through to `stamp`. */
    const old = path.join(DIST, 'sitemap.xml');
    if (fs.existsSync(old)) {
      const xml = fs.readFileSync(old, 'utf8');
      const byLoc = new Map(
        [...xml.matchAll(/<loc>([^<]+)<\/loc>\s*<lastmod>([^<]+)<\/lastmod>/g)]
          .map((m) => [m[1], m[2]])
      );
      for (const p of cfg.pages.filter((x) => x.index !== false)) {
        const date = byLoc.get(`${URL}/${publicPath(p.file)}`);
        /* Hash intentionally left blank: it forces the first comparison below
           to treat the page as changed ONLY if its bytes differ from what the
           date claims — which we cannot know — so instead we record the hash
           we see now and keep the inherited date. */
        if (date) store[p.file] = { hash: null, date };
      }
    }
  }
  const dates = {};
  ...
  for (const p of cfg.pages.filter((x) => x.index !== false)) {
    ...
    const prev = store[p.file];
    if (!prev) {
      store[p.file] = { hash, date: stamp };
      dirty = true;
    } else if (prev.hash === null) {
      /* Seeded from the old sitemap: adopt the bytes we can see, keep the date
         we inherited. */
      store[p.file] = { hash, date: prev.date };
      dirty = true;
    } else if (prev.hash !== hash) {
      store[p.file] = { hash, date: stamp };
      dirty = true;
    }
    dates[p.file] = store[p.file].date;
  }
  if (dirty) {
    store._note = 'GENERATED by tools/build-deploy.js. Do not hand-edit — it records when each page last actually changed, so sitemap.xml can say so honestly. Committed on purpose: a fresh clone must inherit the dates.';
    fs.writeFileSync(LASTMOD, `${JSON.stringify(store, null, 2)}\n`);
  }
  return dates;
};

/* --- sitemap.xml ----------------------------------------------------------- */
const sitemap = (dates) => `<?xml version="1.0" encoding="UTF-8"?>
...
function run(stamp) {
  ...
  /* AFTER the injections, and that ordering is the whole point: the sitemap's
     dates come from hashing the finished pages, so anything written into them
     above has to be there before the hash is taken. It used to be written
     first, which was harmless only because the date it wrote was the clock. */
  if (URL) {
    fs.writeFileSync(path.join(DIST, 'sitemap.xml'), sitemap(lastmodMap(stamp)));
