...
| Headers | 🟡 nosniff, referrer policy and permissions policy ship; **CSP still to add** — and it is now worth adding precisely because there is exactly one third-party script to allow |
| Third-party content | 🔴 twelve hotlinked images remain (P0-2) — the one place a stranger can change what renders inside the page |
