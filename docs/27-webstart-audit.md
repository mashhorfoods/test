...
> **RE-VISITED 7 SEPTEMBER 2026 — `docs/116`.** This table was written on
> 3–4 September and eight of its rows have moved since. **Every one of them
> moved in the same direction: the audit understates where the project is.**
> The single most misleading row is **19**, which still reads *"the four live
> checks remain unrun"* — they were run on 4 September and passed, which is
> why `site.config.json` carries `"hsts": true` today. Amendments are inline
> below, each dated. `docs/116` is the consolidated picture.
...
| **02 Competitive intelligence** | 🟡 PARTIAL *(was ❌ — executed 3 Sep 2026)* | `docs/29-competitive-intelligence.md`: 6 direct + 1 price-band peer + 2 aspirational, benchmarked on pricing, trust, channel, content and scope clarity **Re-checked against 2026 published rates, 4 Sep 2026** (`docs/29` §11): the new ladder's websites now sit inside the Saudi market band — §1's 8×–27× gap is closed — but **branding is still 2.2×–11× below the published full-identity band**, roughly five times the discount websites carry. One question for the owner, one number in `pricing.json` if the answer is no | §1 benchmarked the **old** price list, which P0-6 replaced the same week — that gap is now closed and re-measured. **Branding was discounted about five times harder than websites; repriced the same day to $490/$990/$1,990** (`docs/31` §5.5) — the discount is now even across all four categories. UI/interaction quality is still unverified (page fetches remain blocked). **Egypt is now represented** (`docs/29` §12), and the $490 the branding reprice was anchored on was confirmed against an independent Egyptian source | Spot-check the five pages in §7. Watch branding enquiries in Plausible — `docs/31` §5.5 names it as the one number this change puts at risk |
...
| **10 Homepage design** | 🟡 PARTIAL *(re-checked 4 Sep 2026)* | Nine sections, one `<h1>`, 13 `<h2>`, coherent hierarchy; hero, services, five service details, add-ons, process, CTA, contact; verified in-browser at four widths Proof now exists — a named client case study, a verification band with a real person and two checkable profiles, and a published price list. **Proof on the homepage: DONE 4 Sep 2026** — not as the strip this row asked for. A grid of one reads as a portfolio that failed; a sentence naming a real client does not. `.c-proof` sits under the services list at ~2.5 screens. The strip still needs a second case study to look like a body of work rather than a single job. ~~Still no FAQ~~ — **FAQ shipped 7 Sep, six entries in both languages (`docs/110`)**. Section order is still unvalidated against a real user, and that is B5's. **Also since: the homepage now says who it serves in the hero, the value proposition moved from section 9 to section 2, and Selected Work carries seven real photographs (`docs/109`, `docs/114`)** | Second case study → homepage proof strip; run the five-user script |
| **11 Inner pages & flows** | 🟡 PARTIAL *(re-checked 4 Sep 2026)* | `story.html` (a real client case study), `about.html`, `pricing.html`, `privacy.html`, `404.html`, plus `styleguide.html` (internal, not deployed). Six pages ship | **Terms DONE 4 Sep 2026** (`terms.html`, bilingual, consistent with `pricing.json` `terms.shared`). No `/work` index — with one case study there is nothing to index yet. No per-service pages. **Amended 7 Sep: eight public pages ship now, not six** — `accessibility.html` was added, and `styleguide.html` remains internal | Second case study opens `/work`. A lawyer should read the Terms, especially cancellation |
...
| **13 Responsive & accessibility** | ✅ COMPLETE *(re-verified across all six pages, 4 Sep 2026)* | `docs/16-qa.md` (measured, both directions) plus this audit's independent browser pass: **no horizontal overflow at 360/390/768/1440**, **0 interactive targets below the 44px floor** out of 97, **0 console errors**, one `<h1>`, `<main>`/`<nav>`/`<header>`/`<footer>` landmarks present, skip link present, focus ring at every stop (`src/scripts/focus.js`), 23 `prefers-reduced-motion` guards, contrast failures fixed in Stage 16, RTL verified (`dir`/`lang` flip, no overflow, preference persisted) | ~~no automated axe report committed~~ — **`tools/a11y.js` runs axe-core on every check, 0 violations.** ~~The 12 images carry `alt` but no `width`/`height`~~ — **re-measured 7 Sep: 40 images across the built pages, 0 missing dimensions.** Arabic still needs a native-speaker review (`docs/20`), and **the screen-reader pass is still outstanding** — `docs/67` is the brief. **New since, and found by measurement not review: nine pricing cards were clipped at 320px** (`docs/115`); a fourth harness, `tools/responsive.js`, now renders 320/768/1024 in both languages | Commission the Arabic review; run the screen-reader pass |
| **14 Development architecture** | ✅ COMPLETE *(was 🟡 — closed 4 Sep 2026)* | `docs/61-architecture-decisions.md`: nine decisions with context, consequences and the specific thing that should reverse each — static site, zero runtime dependencies, progressive enhancement, tokens + `@layer`, data files as the CMS, one file per page, manual deploy with automated verification, staging, and error tracking. Plus `.github/workflows/check.yml`: build, a staleness check on the committed `dist/`, and all **four** harnesses on every push *(three until `responsive.js` landed 7 Sep)* | Four of this row's original gaps are closed (monitoring, backups, CI, environments). **No backend, no API, no database and no error tracking remain true and are now recorded as decisions rather than omissions** — AD-01 and AD-09, each with its reversal trigger | Nothing. The reversal triggers are the review |
| **15 Admin dashboard** | ⬛ OUT OF SCOPE *(owner decision, 4 Sep 2026 — under reconsideration)* | C-8: one maintainer who edits the data files directly. `docs/36` §4 records the reasoning, the four triggers, and the four questions that would decide its shape | Not a gap: a dashboard would add a login, a database and an attack surface between one person and a JSON file they already edit. ~~The owner is reconsidering; the decision is no longer settled~~ — **RESOLVED 5 Sep 2026, `docs/63`: the four questions were answered and the answer was Option 0.** GitHub's web editor *is* the dashboard; CI classifies a data-only push, runs the harnesses, rebuilds and commits `dist/` back. **Tested end to end on the live branch, not reasoned about.** There is no dashboard phase still coming | Nothing. Reopen only if a second editor appears |
...
| **18 QA & release readiness** | ✅ COMPLETE *(was 🟡 — executed 4 Sep 2026)* | `docs/43-qa.md` + `tools/qa.js`: data integrity against `pricing.json`, SEO, accessibility with composited contrast, bilingual parity, performance and state checks — against `dist/`, the artefact that ships. Zero findings on the final run | **Four harnesses now** *(three until 7 Sep)* — `validate.js`, `qa.js` (30 sections, adding the showpiece, width-parity, transfer and print budgets), `responsive.js` (320/768/1024 × 8 pages × both languages) and `a11y.js` (axe-core, two widths, both languages, drawer open). **Browser floor documented and two real cross-engine defects fixed** (`docs/59`). Still: no second engine here, no screen-reader pass | The manual device pass in `docs/59` §5 |
| **GATE 03 — release approval** | ✅ **HELD — plain go** *(was a conditional go; condition discharged 5 Sep 2026)* | `docs/46-gate-03.md` + §9: all acceptance criteria measured and met, no critical defect open, rollback in place. **The uptime monitor — the one criterion the gate was held with unmet — is configured, and the four post-upload checks passed.** Re-measured 5 Sep: nothing regressed, and two criteria are now measured more strictly (axe-core; the CSP re-derived from shipped bytes) | Nothing outstanding for this gate. Search Console and the Plausible goals remain, and both block Phase 20 rather than the gate | Phase 20, once there is data |
| **19 Launch & handover** | 🟡 PARTIAL *(was 🔴 — executed 4 Sep 2026)* | `docs/44-launch.md`: the three surfaces distinguished, the release runbook, the four post-upload checks, the HSTS trigger, the domain-migration procedure, and the owner's day-to-day handover with pre-upload commands. `.nojekyll` committed for the review surface | ~~The four live checks remain unrun~~ — **THIS ROW IS THE ONE THAT WENT STALE. The checks were run on 4 September 2026 and passed**, which is what `site.config.json`'s own comment records and why `"hsts": true` shipped: the field is documented as staying false until check 4 shows the live headers. Re-run 5 Sep (`docs/62` A9). **HSTS on · Search Console verified and sitemap submitted (A6) · UptimeRobot with a keyword check (A3) · Plausible goals live (A7)** — all four dependencies closed 5 Sep. This environment still cannot reach the domain; that is why the evidence is the owner's report and the config, not a fetch | **Nothing. Phase 19 is complete.** The one address item left is A10, and it is not a launch step — see below |
| **20 Post-launch optimization** | 🟡 **RUNNING** *(was ❌, then prepared 4 Sep; instrumented 5 Sep 2026)* | `docs/45-optimization.md`: Plausible goal setup, a KPI rulebook that pre-commits what each number changes, six falsifiable hypotheses, and a monthly review order. Ships the analytics queue stub so early events are not dropped. **The four goals and custom properties went live 5 Sep 2026**, so the rulebook has numbers to read for the first time | ~~No data exists~~ — instrumentation is live. What is missing now is **time**: thirty days of it. That is a wait, not a task. **Counted 7 Sep: instrumented 5 September, so the first monthly review falls due around 5 October — two days elapsed, twenty-eight to go** | Collect 30 days, then run the monthly review in `docs/45`. Nothing to do until then |
...
- P1-6 CSP — **DONE 4 Sep 2026**, build-generated from inline-script hashes, zero violations across six pages. **Backup + monitoring plan DONE** — `docs/57`; it needs three owner actions (a second git remote, an uptime monitor with a keyword check, registrar auto-renew) and the monitor **was configured on 5 Sep 2026, closing the Gate 03 criterion**. **Threat model DONE** — `docs/58`; it names three actions worth taking (2FA and a transfer lock on the registrar, host and GitHub; SPF/DKIM/DMARC when domain email exists; a quarterly search for a cloned site) and records that most of OWASP does not apply here by architecture rather than by control. **P1-6 is closed.**
- ~~P1-7 Commit the QA harness~~ **DONE** — three harnesses now, all re-runnable after every change: `tools/validate.js` (the journeys), `tools/qa.js` (the built files, plus the showpiece, width-parity, transfer and print budgets) and `tools/a11y.js` (axe-core). **Cross-browser closed as far as it can be from here, 4 Sep 2026** — `docs/59` states the support floor (Chrome 105 / Safari 16 / Firefox 121), names `@layer` as the one catastrophic dependency, and fixed two real defects the analysis found: `backdrop-filter` losing the scrolled header on every iPhone below Safari 18, and a `:has()` focus ring that left keyboard users on Firefox <121 with no indicator at all. **A second engine still cannot be run here** — the network policy blocks Playwright's browser downloads — so `docs/59` §5 is a manual pass. **It was run by the owner on 5 Sep 2026 with nothing reported wrong, but without per-row notes**, which `docs/59` records as *run* rather than *verified*; §5a reduces what remains to two thirty-second checks.
  `story.html` and `404.html` to homepage depth.
- P1-8 SEO — canonical, `og:url`, sitemap and robots ship and are verified by `tools/qa.js`. **`og:image` + Twitter card, Organization JSON-LD and `apple-touch-icon` all DONE 4 Sep 2026** — the card and the icon are generated (`npm run card`, `npm run icon`) from source that already exists rather than exported by hand, and `qa.js` asserts both shipped and neither is a `data:` URI. ~~Outstanding: Search Console only.~~ **Search Console verified and the sitemap submitted, 5 Sep 2026. P1-8 is CLOSED.**
