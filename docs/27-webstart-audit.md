...
| **01 Discovery & business analysis** | ❌ NOT DONE | Service and package data (`src/data/pricing.json`) is real and approved; the "no invented content" rule is honoured throughout | No business goals, no audience definition, no market/constraint analysis, no existing-site audit, no analytics evidence. The word "discovery" appears nowhere in 27 stage documents | Run compressed discovery (§7 Phase 01 brief) |
| **02 Competitive intelligence** | ❌ NOT DONE | None. Zero references to competitors in the repository | Every UI and IA decision so far was made without a benchmark. We cannot claim "competitive" without one | Benchmark 5–7 direct + 2–3 aspirational agencies |
| **03 Problem → insight → solution** | ❌ NOT DONE | Design decisions are well argued *internally* (e.g. `docs/18-refinement.md` removed duplicated pricing) but argued from craft, not from user or business problems | No product rationale exists. Features are justified by taste and consistency, not by evidence | Produce the Problem → Insight → Solution → Feature → UX Outcome table |
| **04 Strategy & KPIs** | ❌ NOT DONE | Implicit positioning ("one partner") is stated in the hero and holds across the page | No primary/secondary goals, no conversion strategy, no KPIs, no scope boundaries. Nothing on this site is measurable — there is no analytics of any kind | Define positioning, goals, conversion model, KPIs, out-of-scope list |
| **05 Content & IA** | 🟡 PARTIAL | `src/scripts/navigation-map.js` is a single source of section order and labels; `sitemap.xml`; nine homepage sections + `/story.html`; full bilingual copy in `src/data/i18n-ar.json` | No content inventory, no formal sitemap document, no user flows, no page hierarchy beyond "one long page". No About, no legal pages, no proof/case-study structure. Content states (empty/error/loading) undefined | Produce content inventory + sitemap + the two primary user flows |
| **06 Technical discovery & CMS/data architecture** | 🟡 PARTIAL | Deliberate, documented front-end stack (no framework, no dependencies); `site.config.json`; data lives in JSON (`pricing.json`, `story.json`, `i18n-ar.json`) with generators in `tools/` | No data model beyond those three files, no CMS decision, no API/integration plan, no auth requirements, no admin capability definition, no environments, no analytics/SEO tooling decision. Security requirements were never written down at this phase | Decide platform direction, data model, and dashboard requirements (not the dashboard itself) |
| **07 UX wireframes** | ❌ NOT DONE | One passing mention in `docs/03-services.md` | No wireframes exist for any template or flow. Layout was solved directly in high-fidelity HTML | Wireframe only what §7 shows is changing — not the whole site |
| **GATE 01 — structural approval** | ❌ NOT HELD | — | High-fidelity UI and production code were built without a frozen structural foundation. This is the single largest process defect in the project | Hold Gate 01 after phases 01–05 land |
| **08 Master prompt** | 🟡 PARTIAL | `docs/00-design-system.md` §1–18 is an excellent visual/UX instruction set: tokens, grid, type, motion, interaction states, responsive rules, accessibility rules, RTL rules, naming, cascade, governance | Missing the half that comes from phases 01–04: brand DNA in words, audience, problems, strategy, IA and explicit prohibitions | Extend `docs/00` into the full master prompt once 01–04 exist |
...
| **15 Admin dashboard** | ❌ NOT DONE | None. Content is edited by hand in HTML/JSON, then rebuilt | Nothing on the site is editable by a non-technical owner. Every price change requires a developer, a build and an upload | **Do not build yet.** Define requirements in Phase 06 first (§8) |
| **16 Security / secure SDLC** | 🟡 PARTIAL | Genuinely good for a static site: zero third-party JS, zero dependencies (no supply chain to audit), no `innerHTML`/`onclick`/`onerror` anywhere, `.htaccess` sets `X-Content-Type-Options`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` denying camera/mic/geo, HTTPS redirect, and HSTS deliberately withheld until TLS is confirmed | No CSP. HSTS still off although the domain is now live. **Twelve images are hotlinked from `i.ibb.co`** — a free host that can change or serve anything at those URLs and that sees every visitor's IP. Contact email is published in plaintext (harvesting) and is a personal Gmail. No threat model, no logging, no backups, no rate limiting (nothing to rate-limit yet, because there is no backend) | Self-host the images, add a CSP, enable HSTS once TLS is verified, write the threat model |
...
| **18 QA & release readiness** | 🟡 PARTIAL | `docs/16-qa.md`: ten measured groups — overflow at twelve widths, zero dead anchors, contrast, target size, keyboard walk, all 28 package figures, one currency per language, no console errors, readable with JS off | The audit scripts are **not in the repository** (they lived in an untracked scratchpad), so none of it is repeatable or regression-proof. No cross-browser matrix (Chromium only), no real-device testing, no performance budget enforced in CI, `story.html` / `404.html` never audited to the same depth | Commit the QA harness; add a browser/device matrix |
...
| P0-2 | Self-host the twelve `i.ibb.co` images | Third-party control of in-page content + visitor IP leakage + a live break the moment that host changes policy |
...
| P0-4 | Fix the lead path: a capture that works without a mail client, and an agency-owned address | Lost enquiries are lost revenue; the site's only conversion currently has a silent failure mode |
| P0-5 | Install analytics with one defined conversion event, plus a privacy policy covering it | Without it Phase 20 is impossible and the contact form collects personal data with no stated basis |

### P1 — Important (required to meet the WEBSTART standard)

- P1-1 Proof and trust: clients, results, testimonials — real and permissioned.
...
- P1-7 Commit the QA harness; add a cross-browser/device matrix; audit
