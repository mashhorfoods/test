...
**Score:** 5 ✅ · 12 🟡 · 3 ❌ · 1 🔴 (+ 3 gates unheld) — updated after Phases 01–05 and 07 ran; see `docs/28`–`docs/33`. Every structural phase now has its output; Gate 01 is an approval decision, not a work item
...
| P0-2 🔴 | Self-host the twelve `i.ibb.co` images — **blocked in this environment**: the egress policy refuses that host, so the files cannot be fetched here. Mitigated meanwhile with lazy loading, async decoding and a ground colour, so an outage degrades to quiet empty panels | Third-party control of in-page content + visitor IP leakage + a live break the moment that host changes policy |
...
| P0-5 **half** | Four events are instrumented and verified (`src/scripts/analytics.js`); nothing is sent until a provider is named in `site.config.json`. The privacy policy is deliberately unwritten until the provider and the entity (C-6) exist | Without it Phase 20 is impossible and the contact form collects personal data with no stated basis |
