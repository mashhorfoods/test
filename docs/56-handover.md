...
| The hero film | `src/showpiece/scene.html`, then `npm run film` | Or `npm run film:clips -- clipA.mp4 clipB.mp4` for video. **What ships today is the video, and it carries a Pika watermark** — a deliberate licence decision, `docs/54` §9 and `docs/69` B7. `npm run film` reverts to the drawn film and the watermark with it |
