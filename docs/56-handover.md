...
| The hero film | `src/showpiece/scene.html`, then `npm run film` | **What ships is the drawn film — ours outright, no licence question** (`docs/54` §10). `npm run film:clips -- clipA.mp4 clipB.mp4` wraps supplied footage instead; `qa.js` §23 flags that path as one to check for watermarks before shipping. Every build signs `provenance.json` |
