...
| ~~**E4**~~ | ~~Split-bundle option for repeat visits.~~ **BUILT 5 Sep 2026 — `docs/43` §15, AD-06 amended.** `site.config.json` `build.fonts: "linked"` copies the faces to `assets/fonts/`; both modes pass all three harnesses and `qa.js` §13 fails on a page referencing a face the zip lacks. **AD-06's revisit trigger was the wrong question.** Fonts are 46% of every gzipped page, and inlining them defeats the `unicode-range` that `00-fonts.css` was written around — a data URI is not a download, so **every English visitor pays for the 30.9KB Arabic face they never see.** Linked, a first-time English visitor reading ONE page sends 106,577 bytes instead of 139,946 — 24% less with no cache involved; later pages 57–62% less. **Now the default**, on your call to decide it. The fallback-flash objection was measured rather than assumed: first contentful paint on a simulated 1.6Mbps/150ms mobile connection is **440ms linked against 648ms inline** — separate files paint 32% SOONER, because the inlined font bytes were themselves render-blocking. The flash itself is answered by the preload tags `index.html` has carried since it was written and the build used to strip |
| **E5** | An FAQ section, if the sales conversation turns out to repeat questions |
| ~~**E6**~~ | ~~Prune the CSS that ships to nobody.~~ **DONE 5 Sep 2026 — `docs/43` §14.** It contained an accessibility defect: `01-reset.css`'s `ul[role="list"]` idiom was in the dead list, and the reason was that **125 of 164 lists remove their marker and not one carried `role="list"`** — so on VoiceOver the navigation, the footer and every package's feature list were runs of text rather than lists. axe reports zero violations there; it is a Safari behaviour, not a rule. Fixed at all three sources of truth, layout identical on every page and width, and `qa.js` now fails on a regression. The pruning itself: `card.css` and `disclosure.css` had **zero live rules** across the shipped pages and now load only from `showroom.css`; six leftover rules deleted; dead selectors **163 → 91**; index.html **141,173 → 139,946 bytes gzipped**. The remaining 91 stay deliberately — removing all of them was measured at ~2.5KB/page (2%), several are published API in `docs/00`, and **X09 is Design System Refinement**, which is the phase that should decide the system's shape |
