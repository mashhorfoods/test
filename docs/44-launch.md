...
**Upload** the zip to `public_html` and extract it there. Eleven files plus
`assets/` — *(updated 7 Sep 2026; it was nine files and 12 images when this
was written, and the list is generated by the build rather than typed)*:

```
index.html  pricing.html  about.html  story.html  privacy.html
terms.html  accessibility.html  404.html
.htaccess   robots.txt    sitemap.xml   assets/ (46 files)
```

`styleguide.html` is built but internal — it is not part of the upload.
...
| Deployment package complete, dotfile included | ✅ eleven files + `assets/` |
| **Live TLS, redirect, compression, rewrite, 404 route** | ✅ **checked 4 September 2026 by the owner and passed**, re-run 5 Sep (`docs/62` A9). Still not verifiable *from here* — the egress policy blocks both domains — so the evidence is the owner's report plus the config that depends on it |
| HSTS | ✅ **on.** `site.config.json` `"hsts": true`, and that field's own comment records that it stayed false until check 4 passed |
| Analytics receiving events | ✅ the four Plausible goals and custom properties went live 5 Sep (A7) |
| Search Console, uptime monitor | ✅ property verified and `sitemap.xml` submitted (A6); UptimeRobot with a keyword check (A3) |

**Phase 19 is complete.** *(Amended 7 September 2026 — this section previously
read "complete on everything that can be done from here", with the four checks
outstanding in red. They were run on 4 September. The row above was the last
thing in the project still describing the site as unlaunched; `docs/116` is the
consolidated revisit.)*
...
input that does not exist yet: the output of the four checks.~~

**Closed.** The gate was held on 4 September on the §6 condition, and
`docs/46` §9 records it becoming a plain go on 5 September when A3 — the
uptime check — closed the last unmet criterion. **No gate in this project is
now held on an open criterion.**
