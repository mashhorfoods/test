...
| ~~**B7**~~ | ~~You~~ | **DONE 6 Sep 2026 — `docs/54` §10.** The drawn film ships; `npm run film`, one command. **Nothing was removed or covered** — the drawn scene replaces the footage outright, so no licence condition is circumvented. Verified by reading frames rather than trusting the command: 9 samples across `hero.webm`, plus `hero.mp4` and `hero-poster.webp`, all clean in the exact region the logo occupied. Mirrors correctly at `[dir="rtl"]`. 963KB/403KB, 47% of budget; poster 10KB, still inlined. **`qa.js` §23 now reads a `provenance.json` each builder signs** — the missing mechanism that let a commit-message decision ship for two days | ✅ |
