...
| **B7** | You | **The Pika watermark on the hero — a decision, not a bug.** Clips A and B ship as exported, watermark visible, because on a free tier that mark *is* the licence condition; the commit that shipped them said "replace them with clean exports later" and that half was never tracked anywhere. It is in `hero.webm`, `hero.mp4` **and `hero-poster.webp`** — the poster is what every phone, reduced-motion and no-JS visitor sees instead of the film, so for them the mark is the hero, permanently. Three ways out in `docs/54` §9: **revert to the drawn film** (`npm run film`, one command, 211KB, no licence question), a **paid watermark-free export** (`npm run film:clips`, also one command), or **ship as is** knowingly. Removing it is not on the list | The hero stops carrying another company's wordmark |
